...
private:
    ...
    int hashFunction2(int key) {
        int R = nearestPrimeNum(tableSize);
        return R - (key % R);
    }
    ...
    int getProbeIndex(int index, int i, int key) {
        if (choice == 1) {
            ...
            return (index + i * hashFunction2(key)) % tableSize;
        }
...
public:
    ...
    void Insert(int key) {
        ...
        while (i < 6) {
            int newIndex = getProbeIndex(index, i, key);
            if (table[newIndex] == -1) {
                table[newIndex] = key;
                numOfItems++;
